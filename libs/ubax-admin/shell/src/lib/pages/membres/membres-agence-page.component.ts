...
export class MembresAgencePageComponent implements OnInit {
  ...
  private readonly store = inject(AdminAgencyMembersStore);
  ...
  protected readonly pageTitle = computed(() => {
    switch (this.memberScope()) {
      case 'inactive':
        return "Membres inactifs de l'agence";
      case 'all':
        return "Tous les membres de l'agence";
      default:
  ...
  protected readonly pageSubtitle = computed(() => {
    switch (this.memberScope()) {
      case 'inactive':
        return 'Consultez les profils désactivés, avec leur date de retrait de la plateforme.';
      case 'all':
        return "Vision consolidée de l'équipe active et des comptes désactivés.";
      default:
        return "Vue en lecture seule — la gestion de l'équipe est assurée par le Directeur d'agence.";
    }
  });
  ...
  protected readonly emptyDescription = computed(() => {
    ...
    switch (this.memberScope()) {
      case 'inactive':
        return "Cette agence n'a aucun membre inactif.";
      case 'all':
        return "Cette agence n'a encore aucun membre.";
      default:
        return "Cette agence n'a pas encore de membres actifs.";
    }
  });
  ...
  private async loadMembers(): Promise<void> {
    ...
    try {
      await this.store.load(this.agencyId, {
        scope: this.memberScope(),
      });
    } catch {
      this.notif.error(
        this.store.error() ?? "Impossible de charger les membres de l'agence.",
      );
    }
  }
