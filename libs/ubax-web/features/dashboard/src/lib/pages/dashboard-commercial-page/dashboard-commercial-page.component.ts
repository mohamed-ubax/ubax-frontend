...
export class DashboardCommercialPageComponent {
  ...
  readonly calEvents: CalEvent[] = [
    {
      label: 'Visite appartement Cocody',
      status: 'confirmed',
      dayIndex: 0,
      startSlot: 1,
      span: 2,
    ...
    {
      label: 'Rendez-vous client Plateau',
      status: 'upcoming',
      dayIndex: 1,
      startSlot: 3,
      span: 1,
  ...
  getEventsForSlot(dayIndex: number, slotIndex: number): CalEvent[] {
    return this.calEvents.filter(
      (e) => e.dayIndex === dayIndex && e.startSlot === slotIndex,
    );
  }
