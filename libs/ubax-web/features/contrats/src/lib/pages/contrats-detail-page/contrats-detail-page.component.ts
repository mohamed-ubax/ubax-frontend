...
export class ContratsDetailPageComponent {
  ...
  readonly heroMetrics = computed<DetailTile[]>(() => {
    ...
    return [
      ...
      },
      {
        ...
        hint: c.startDate ? `À partir du ${this.formatDate(c.startDate)}` : undefined,
        icon: 'pi-calendar',
      },
      {
        label: 'Paiement attendu',
        value: this.formatPaymentDay(c.paymentDay),
        hint: c.endDate ? `Échéance le ${this.formatDate(c.endDate)}` : 'Sans fin définie',
        icon: 'pi-clock',
      },
    ];
  });
    ...
    return [
      {
        ...
        hint: this.joinText([c.tenantEmail, c.tenantPhone], ' · ') ?? 'Coordonnées non transmises',
        icon: 'pi-user',
        tone: 'accent',
      },
      {
        ...
        hint: c.referenceNumber ? `Référence ${c.referenceNumber}` : 'Titulaire du bien',
        icon: 'pi-briefcase',
      },
      {
        label: 'Bien concerné',
      ...
      },
      {
        ...
        hint: c.createdById ? `Création ${this.shortId(c.createdById)}` : 'Pilotage du dossier',
        icon: 'pi-building',
      },
      {
        label: 'Nature du contrat',
        value: this.getContractTypeLabel(c.contractType),
        hint: c.status ? `Statut ${this.getStatusLabel(c.status).toLowerCase()}` : 'Type contractuel',
        icon: 'pi-bookmark',
      },
    ];
  });
  ...
  readonly financialItems = computed<DetailTile[]>(() => {
    ...
    if (!c) return [];
    return [
      { label: 'Loyer mensuel', value: this.formatAmount(c.monthlyRent), hint: 'Base locative hors charges', icon: 'pi-home' },
      { label: 'Charges mensuelles', value: this.formatAmount(c.monthlyCharges), hint: 'Charges récupérables du bail', icon: 'pi-bolt' },
      { label: 'Mensualité globale', value: this.formatAmount(this.resolveTotalMonthlyAmount(c)), hint: 'Montant facturé chaque mois', icon: 'pi-credit-card', tone: 'accent', featured: true },
      { label: 'Dépôt de garantie', value: this.formatAmount(c.depositAmount), hint: c.depositReturned ? 'Restitué' : 'Toujours immobilisé', icon: 'pi-shield', tone: c.depositReturned ? 'success' : 'warning' },
      { label: 'Commission agence', value: this.formatPercentage(c.agencyCommissionRate), hint: 'Taux prévu au contrat', icon: 'pi-percentage' },
      { label: 'Jour de paiement', value: this.formatPaymentDay(c.paymentDay), hint: 'Cadence de recouvrement locatif', icon: 'pi-calendar' },
    ];
  });
  ...
  readonly summaryRows = computed<SummaryRow[]>(() => {
    ...
    if (!c) return [];
    return [
      { label: 'Référence', value: c.referenceNumber ?? `CTR-${this.shortId(c.id)}`, hint: `Identifiant interne ${this.shortId(c.id)}` },
      { label: 'Début du bail', value: this.formatDate(c.startDate), hint: c.createdAt ? `Créé le ${this.formatDateTime(c.createdAt)}` : undefined },
      { label: 'Fin du bail', value: this.formatDate(c.endDate), hint: c.updatedAt ? `Mis à jour le ${this.formatDateTime(c.updatedAt)}` : undefined },
      { label: 'Durée estimée', value: this.getContractDurationLabel(c.startDate, c.endDate) },
      { label: 'Commission agence', value: this.formatPercentage(c.agencyCommissionRate) },
      { label: 'Restitution caution', value: c.depositReturned ? 'Effectuée' : 'Non restituée', hint: this.formatAmount(c.depositAmount) },
    ];
  });
    ...
    return [
      {
        ...
        hint: c.signedFileUrl ? 'Version prête au téléchargement' : 'Le document sera attaché automatiquement',
        icon: 'pi-file-pdf',
      ...
      },
      {
        ...
        hint: c.status === 'ACTIVE' ? 'Historique et règlements disponibles' : 'Disponible dès activation du bail',
        icon: 'pi-history',
  ...
  getContractTypeLabel(type: ContractResponse['contractType'] | undefined): string {
    return type ? this.CONTRACT_TYPE_LABELS[type] : 'Contrat';
  }

  getStatusContext(status: ContractStatus | undefined): string {
    switch (status) {
      case 'DRAFT': return 'Bail en préparation, encore modifiable avant envoi.';
      case 'PENDING_SIGNATURE': return 'Bail prêt et en attente de validation finale.';
      case 'ACTIVE': return 'Bail actif avec échéances et loyers en cours.';
      case 'TERMINATED': return 'Bail clôturé par résiliation.';
      case 'EXPIRED': return 'Bail arrivé à son terme contractuel.';
      case 'CANCELLED': return 'Bail annulé avant mise en application.';
      default: return 'Dossier contractuel en cours de suivi.';
    }
  }

  getContractDurationLabel(startDate: string | null | undefined, endDate: string | null | undefined): string {
  ...
  private resolveTotalMonthlyAmount(contract: ContractResponse | null | undefined): number | undefined {
    if (!contract) return undefined;
    if (contract.totalMonthlyAmount != null) return contract.totalMonthlyAmount;
    if (contract.monthlyRent == null && contract.monthlyCharges == null) return undefined;
    return (contract.monthlyRent ?? 0) + (contract.monthlyCharges ?? 0);
  }

  private buildMonthlyAmountHint(contract: ContractResponse | null | undefined): string | undefined {
    if (!contract) return undefined;
    return this.joinText(
      [
        Number.isFinite(contract.monthlyRent) ? `Loyer ${this.formatAmount(contract.monthlyRent)}` : undefined,
        Number.isFinite(contract.monthlyCharges) ? `Charges ${this.formatAmount(contract.monthlyCharges)}` : undefined,
      ],
      ' + ',
    );
  }

  private joinText(values: Array<string | null | undefined>, separator = ', '): string | undefined {
