...
const DOCUMENT_SEED_ROWS: readonly ArchivageRow[] = Array.from(
  ...
  (_, index) => ({
    ...
    type: 'Contrat',
    cells: [
      { kind: 'text', value: 'Contrat' } as const,
      { kind: 'text', value: 'Immeuble kalia' } as const,
      {
        kind: 'avatar',
        imageSrc: `${PEOPLE_ROOT}/document-01.webp`,
        value: 'Kevin Kouassi',
      } as const,
      { kind: 'text', value: '15/07/2026' } as const,
      { kind: 'text', value: '15/07/2026' } as const,
      { kind: 'badge', label: 'expiré', tone: 'danger' } as const,
      DEFAULT_ACTIONS,
    ],
  }),
);
